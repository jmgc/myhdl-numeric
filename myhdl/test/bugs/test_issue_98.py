...
def test_issue_98_1():
    sda_i, sda_o, scl_i, scl_o = [Signal(False) for _ in range(4)]
...
def test_issue_98_2():
    sda_i, sda_o, scl_i, scl_o = [Signal(intbv(0)[2:0]) for _ in range(4)]
    sda, scl = [TristateSignal(intbv(0)[2:0]) for _ in range(2)]
...
def test_issue_98_3():
    sda_i, sda_o, scl_i, scl_o = [Signal(intbv(0)[1:0]) for _ in range(4)]
    sda, scl = [TristateSignal(intbv(0)[1:0]) for _ in range(2)]
