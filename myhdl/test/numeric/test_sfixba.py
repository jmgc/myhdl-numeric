...
class TestSFixBaInit(TestCase):
    ...
    def testPIntValue(self):
        ...
        self.assertEqual(str(value), "0101", "Wrong binary string " \
                         "high={}, low={}".format(value.high, value.low))
        ...
        for i, f_value, i_value, b, h in zip(cases, f_values, i_values, b_check, h_check):
            ...
            for j in range(value.low, value.high):
                for k in range(value.low, j - 1):
                    ...
                    if float(data) != check:
                        check = resize(f_value, data)
                    self.assertEqual(float(data), check, "value: {}, " \
                                     "fix: {}, float(fix): {}, float: {}, " \
                                     "high:{}, low: {}".format( f_value, data,
                                                                float(data),
                                                                check, j, k))
            ...
            for i in range(0, 8):
                for j in range(delta, i-1):
                    for k in range(-128, 128):
                        ...
                        check = resize(f_value, data)
                        self.assertEqual(data, check, "integer: {}, " \
                                         "delta: {}, i: {}, " \
                                         "j: {}, data: {}, " \
                                         "float: {}".format(k, delta, i,
                                                            j,
                                                            data,
                                                            check))
    ...
    def testFloatPLowValue(self):
        ...
        self.assertEqual(value.__index__(),  i_value, "Wrong value {}, {}".format(value, i_value))
...
class TestSFixBaAsInt(TestCase):
    ...
    def testIPow(self):
        self.augmentedMathAssignCheck(operator.ipow, imin=-512, imax=512,
                                      jmin=-8, jmax=8)
