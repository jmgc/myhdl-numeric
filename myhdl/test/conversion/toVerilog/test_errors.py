...
from myhdl import instance, Signal, intbv, delay, downrange, StopSimulation, \
    toVerilog, Simulation
...
def listComp1(count, enable, clock, reset, n):
    ...
    @instance
    def logic():
        mem = [intbv(0)[8:] for _ in range(4) for _ in range(5)]
        while 1:
            yield clock.posedge, reset.negedge
            count.next = count + 1
    return logic
...
def listComp2(count, enable, clock, reset, n):
    ...
    @instance
    def logic():
        mem = [intbv(0)[8:] for _ in downrange(4)]
        while 1:
            yield clock.posedge, reset.negedge
            count.next = count + 1
    return logic
...
def listComp3(count, enable, clock, reset, n):
    @instance
    def logic():
        mem = [intbv(0)[8:] for _ in range(1, 4)]
        while 1:
            yield clock.posedge, reset.negedge
            count.next = count + 1
