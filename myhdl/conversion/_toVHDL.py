...
class _GenerateHierarchy:
    ...
    def _revisePorts(self, intf, stdLogicPorts):
        ...
        for name, s in intf.argdict.items():
            ...
            if stdLogicPorts and ds._type in (intbv, bitarray, uintba, sintba, sfixba):
                final_name = port.name + "_num"
                convert_port = port.name

                if isinstance(port.vhd_type, vhd_array):
                    vhd_obj = vhd_array(s.depth, vhd_vector(len(port.vhd_type.type)))
    ...
    if port.convert:
        if isinstance(port.vhd_type, (vhd_array, vhd_enum)):
            for sl in port.signal.elObj._slicesigs:
                sl._setName('VHDL')
            if isinstance(port.vhd_type.type, (vhd_vector, vhd_unsigned, vhd_signed, vhd_sfixed)):
                port_type = vhd_array(port.vhd_type.high + 1, vhd_vector(len(port.vhd_type.type))).toStr(False)
            else:
                port_type = "std_logic_vector"
        else:
            for sl in port.signal._slicesigs:
                sl._setName('VHDL')
            port_type = "std_logic_vector(%d downto 0)" % (len(port.vhd_type) - 1)
        ...
        if port.direction in ("inout", "out"):
            if isinstance(port.vhd_type, vhd_array):
                ...
                if isinstance(port.vhd_type, vhd_sfixed):
                    port_conversions.append(vhd_assign(port.name,
                                                       f"to_slv({port.convert})"))
                else:
                    port_conversions.append(vhd_assign(port.name,
                                                       f"std_logic_vector({port.convert})"))
            ...
            if isinstance(port.vhd_type, vhd_array):
                if isinstance(port.vhd_type.type, vhd_sfixed):
                    ...
                    for idx in range(port.vhd_type.high + 1):
                        port_conversions.append(vhd_assign(f"{port.convert}({idx})",
                                                           f"{port.vhd_type.type.toStr(False)}({port.name}({idx}))"))
            else:
                if isinstance(port.vhd_type, vhd_sfixed):
    ...
    for unit_name in entity.architecture.components:
        ...
        if len(component.entity.ports_list) > 0:
            ...
            for port_name in component.entity.ports_list:
                ...
                convert = component.entity.ports_dict[port_name].convert
                if convert:
                    port_name = port.name
                name = component.ports_signals_dict[port_name]
                if convert and isinstance(port.vhd_type, (vhd_unsigned, vhd_signed)):
                    f.write(c)
                    if port.direction == "out":
                        if name == 'open':
                            f.write("%s => %s" % (port_name, name))
                        else:
                            f.write("%s(%s) => %s" % (port.vhd_type.toStr(False), port_name, name))
                    elif port.direction == "in":
                        f.write("%s => std_logic_vector(%s)" % (port_name, name))
                    else:
                        f.write("%s(%s) => std_logic_vector(%s)" % (port.vhd_type.toStr(False), port_name, name))
                elif convert and isinstance(port.vhd_type, (vhd_sfixed)):
                    f.write(c)
                    if port.direction == "out":
                        if name == 'open':
                            f.write("%s => %s" % (port_name, name))
                        ...
                        f.write("%s => to_slv(%s)" % (port_name, name))
                    else:
                        f.write("%s(%s) => to_slv(%s)" % (port.vhd_type.toStr(False), port_name, name))

                elif convert and isinstance(port.vhd_type, vhd_array):
                    if isinstance(port.vhd_type.type, vhd_sfixed):
                        for idx in range(port.vhd_type.high + 1):
                            f.write(c)
                            if port.direction == "out":
                                ...
                                f.write("%s(%d) => to_slv(%s(%d))" % (port_name, idx, name, idx))
                            else:
                                f.write("%s(%s(%d)) => to_slv(%s(%d))" % (
...
class _ConvertVisitor(ast.NodeVisitor, _ConversionMixin):
    ...
    def visit_Call(self, node):
        ...
        if node.args:
            ...
            self.write(opening)
            self.visit(node.args[0])
            if f is sfixba.resize:
                self.write(sep)
                if isinstance(node.args[1].vhd, vhd_nat):
                    self.write('integer(')
                elif isinstance(node.args[1].vhd, vhd_int):
                    self.write('(')
                self.visit(node.args[1])
                if isinstance(node.args[1].vhd, vhd_int):
                    self.write(')-1')
    ...
    def visit_IfExp(self, node):
        ...
        self.write('tern_op(')
        self.write('cond => ')
        if isinstance(node.test.vhd, vhd_std_logic):
            self.write("((")
        self.visit(node.test)
        if isinstance(node.test.vhd, vhd_std_logic):
            self.write(") = '1')")
...
class vhd_vector(vhd_type):
    ...
    def toStr(self, constr=True):
        if constr:
            return "std_logic_vector(%s downto 0)" % (self.size - 1)
        else:
...
class _AnnotateTypesVisitor(ast.NodeVisitor, _ConversionMixin):
    ...
    def visit_Attribute(self, node):
        ...
        if node.attr == 'min':
            if isinstance(node.value.vhd, (vhd_signed, vhd_sfixed)):
                node.vhd = vhd_int(-1)
            else:
                node.vhd = vhd_nat(0)
        elif node.attr == 'max':
            node.vhd = vhd_nat(1)
        elif node.attr in ('high', 'low'):
            node.vhd = vhd_int(-1)
    ...
    def inferCompareType(self, node, left, right):
        ...
        if isinstance(l, vhd_sfixed):
            ...
            if isinstance(r, vhd_unsigned):
                ...
                elif isinstance(right, ast.Attribute) and right.attr in attributes:
                    if isinstance(right.value.vhd, vhd_signed):
                        r = vhd_signed(right.value.vhd.size + 1)
                    elif isinstance(right.value.vhd, vhd_unsigned):
            ...
            if isinstance(l, vhd_unsigned):
                ...
                elif isinstance(left, ast.Attribute) and left.attr in attributes:
                    if isinstance(left.value.vhd, vhd_signed):
                        l = vhd_signed(left.value.vhd.size + 1)
                    elif isinstance(left.value.vhd, vhd_unsigned):
            ...
            if isinstance(r, vhd_nat):
                if isinstance(right, ast.Constant):
                    r = vhd_unsigned(uintba(right.value).high)
                elif isinstance(right, ast.Attribute) and right.attr in attributes:
                    # Max case
                    r = vhd_unsigned(right.value.vhd.size + 1)
                else:
                    r = vhd_unsigned(r.MAX_SIZE)
            elif isinstance(r, vhd_int):
                l = vhd_signed(l.size + 1)
                if isinstance(right, ast.Constant):
                    r = vhd_signed(sintba(right.value).high)
            ...
            if isinstance(l, vhd_nat):
                if isinstance(left, ast.Constant):
                    l = vhd_unsigned(uintba(left.value).high)
                elif isinstance(left, ast.Attribute) and left.attr in attributes:
                    # Max case
                    l = vhd_unsigned(left.value.vhd.size + 1)
                else:
                    l = vhd_unsigned(l.MAX_SIZE)
            elif isinstance(l, vhd_int):
                r = vhd_signed(r.size + 1)
                if isinstance(left, ast.Constant):
                    l = vhd_signed(sintba(left.value).high)
    ...
    def inferBitOpType(self, node):
        ...
        if isinstance(node.op, ast.BitAnd):
            ...
            obj = left ^ right
        obj_result = copy(obj)
        obj_result.trunc = True
        if isinstance(obj.size, tuple):
            obj.size = (obj.size[0] + high, obj.size[1])
        else:
            obj.size += high
        obj.trunc = True
        node.vhdOri = node.left.vhd = node.right.vhd = obj
        node.vhd = obj_result
